...
export const MarkdownEditor = forwardRef<MarkdownEditorRef, MarkdownEditorProps>(
  function MarkdownEditor(
      ...
      editorProps: {
        handleKeyDown: (_view, event) => {
          if (onKeyDownRef.current) {
            onKeyDownRef.current(event as unknown as React.KeyboardEvent);
          }
          return false;
        },
        attributes: {
          class: cn(
            ...
            compact ? "text-[13px] px-2.5 pt-2 pb-1" : "text-sm p-2.5"
          ),
        },
      },
    });
    ...
    const toolbarActions = compact
      ? [
          { icon: Bold, action: () => editor?.chain().focus().toggleBold().run(), title: "Bold", active: "bold" },
          { icon: Italic, action: () => editor?.chain().focus().toggleItalic().run(), title: "Italic", active: "italic" },
          { icon: Code, action: () => editor?.chain().focus().toggleCode().run(), title: "Inline code", active: "code" },
          { icon: FileCode, action: () => editor?.chain().focus().toggleCodeBlock().run(), title: "Code block", active: "codeBlock" },
          {
            icon: Link,
            action: () => {
              if (!editor) return;
          ...
          { icon: Quote, action: () => editor?.chain().focus().toggleBlockquote().run(), title: "Quote", active: "blockquote" },
        ]
      : [
          { icon: Heading2, action: () => editor?.chain().focus().toggleHeading({ level: 2 }).run(), title: "Heading", active: "heading" },
          { icon: Bold, action: () => editor?.chain().focus().toggleBold().run(), title: "Bold", active: "bold" },
          { icon: Italic, action: () => editor?.chain().focus().toggleItalic().run(), title: "Italic", active: "italic" },
          { icon: Code, action: () => editor?.chain().focus().toggleCode().run(), title: "Inline code", active: "code" },
          { icon: FileCode, action: () => editor?.chain().focus().toggleCodeBlock().run(), title: "Code block", active: "codeBlock" },
          ...
          { icon: Quote, action: () => editor?.chain().focus().toggleBlockquote().run(), title: "Quote", active: "blockquote" },
          { icon: List, action: () => editor?.chain().focus().toggleBulletList().run(), title: "Bullet list", active: "bulletList" },
          { icon: ListOrdered, action: () => editor?.chain().focus().toggleOrderedList().run(), title: "Numbered list", active: "orderedList" },
        ];
    ...
    return (
      ...
      >
        ...
        >
          {toolbarActions.map(({ icon: Icon, action, title: t, active }) => (
            <button
              key={t}
              onClick={action}
              className={cn(
