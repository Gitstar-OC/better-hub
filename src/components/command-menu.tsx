...
const KNOWN_PREFIXES = new Set([
  ...
  "trending", "users", "orgs", "dashboard", "api", "collections", "starred",
]);
...
type Mode = "commands" | "search" | "theme" | "code-theme" | "accounts" | "settings" | "model" | "files";
...
export function CommandMenu() {
  ...
  const { codeThemeDark, codeThemeLight, setCodeThemeDark, setCodeThemeLight } = useCodeTheme();
  ...
  // Settings state
  const [settingsData, setSettingsData] = useState<{ ghostModel: string; useOwnApiKey: boolean } | null>(null);
  const [settingsLoading, setSettingsLoading] = useState(false);

  // File tree state (for "Go to file" on repo pages)
  const repoContext = useMemo(() => matchRepoFromPathname(pathname), [pathname]);
  const [fileTree, setFileTree] = useState<{ files: string[]; defaultBranch: string } | null>(null);
  ...
  const fetchSettings = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSettingsData({ ghostModel: data.ghostModel, useOwnApiKey: data.useOwnApiKey });
      }
    } catch {
      // silent
    } finally {
  ...
  useEffect(() => {
    const down = (e: KeyboardEvent) => {
      ...
      if (e.key === "/" && (e.metaKey || e.ctrlKey)) {
        e.preventDefault();
        if (open && mode === "search") { setOpen(false); return; }
        setMode("search");
        setSearch("");
        setOpen(true);
      }
      ...
      if (e.key === "g" && (e.metaKey || e.ctrlKey)) {
        ...
        e.preventDefault();
        if (open && mode === "files") { setOpen(false); return; }
        setMode("files");
        setSearch("");
        setOpen(true);
      }
  ...
  const tools = useMemo(() => [
    ...(globalChat ? [{
      ...
      keywords: ["ai", "chat", "ask", "help", "copilot", "bot"],
    ...
    ...(repoContext ? [{
      ...
      keywords: ["open", "navigate", "find file", "browse"],
    ...
    },
    {
      ...
      keywords: ["dark", "light", "mode", "appearance", "colors", "night", "midnight", "contrast", "style"],
    ...
    },
    {
      ...
      keywords: ["code", "syntax", "editor", "highlight", "dracula", "monokai", "catppuccin", "shiki", "font"],
    ...
    },
    {
      ...
      keywords: ["user", "login", "logout", "sign out", "switch", "pat", "token", "profile"],
    ...
    },
    {
      ...
      keywords: ["preferences", "options", "model", "ghost", "api key", "customize"],
  ...
  const suggestions = useMemo(() => {
    ...
    const items: { id: string; name: string; description: string; icon: typeof Search; action: () => void; keepOpen?: boolean; shortcut?: string; image?: string }[] = [];

    if (repoContext) {
      // Repo page: Go to File, Issues, PRs
      items.push({
        id: "suggest-go-to-file", name: "Go to File",
        description: `${repoContext[0]}/${repoContext[1]}`,
        icon: FileText, action: () => switchMode("files"),
        keepOpen: true, shortcut: "⌘G",
      });
      const base = `/${repoContext[0]}/${repoContext[1]}`;
      items.push({
        id: "suggest-issues", name: "Issues",
        description: "Browse issues", icon: CircleDot,
        action: () => router.push(`${base}/issues`),
      });
      items.push({
        id: "suggest-pulls", name: "Pull Requests",
        description: "Browse pull requests", icon: GitPullRequest,
        action: () => router.push(`${base}/pulls`),
      });
    } else {
      // Dashboard / other pages
      items.push({
        id: "suggest-search", name: "Search Repos",
        description: "Find repositories", icon: Search,
        action: () => switchMode("search"),
        keepOpen: true, shortcut: "⌘/",
      });
      // Top 2 recently visited repos
      const repoViews = recentViews.filter(v => v.type === "repo").slice(0, 2);
      repoViews.forEach(v => items.push({
        id: `suggest-${v.url}`, name: v.title,
        description: v.subtitle, icon: FolderGit2,
      ...
      if (globalChat) {
        items.push({
          id: "suggest-ghost",
          name: globalChat.state.isOpen ? "Close Ghost" : "Open Ghost",
          description: "AI assistant", icon: Ghost,
          action: () => globalChat.toggleChat(),
          shortcut: "⌘I",
        });
      }
  ...
  const commandItems = useMemo(() => {
    const items: { id: string; type: "tool" | "recent" | "repo-nav" | "suggestion"; action: () => void; keepOpen?: boolean }[] = [];
    // Suggestions first
    suggestions.forEach((s) =>
      items.push({ id: s.id, type: "suggestion", action: s.action, keepOpen: s.keepOpen })
    );
    ...
    if (hasQuery) {
      repoNavItems.forEach((item) =>
        items.push({ id: `repo-nav-${item.name}`, type: "repo-nav", action: () => router.push(item.href) })
      );
    }
    // Recently viewed
  ...
  // --- Accounts mode items ---
  const handleSwitchAccount = useCallback(async (accountId: string | null) => {
    try {
  ...
  const accountItems = useMemo(() => {
    ...
    items.push({
      id: "account-add",
      action: () => { setAddingAccount(true); setPatInput(""); setPatError(""); },
      keepOpen: true,
    });

    // Quick links
    const activeLogin = accountsData.accounts.find((a) => a.active)?.login || accountsData.oauthLogin;
    items.push({ id: "account-config", action: () => switchMode("settings"), keepOpen: true });
    items.push({
      id: "account-signout",
      action: () => signOut({ fetchOptions: { onSuccess: () => { window.location.href = "/"; } } }),
      keepOpen: false,
    });
    items.push({ id: "account-profile", action: () => window.open(`https://github.com/${activeLogin}`, "_blank"), keepOpen: false });
    items.push({ id: "account-set-pat", action: () => { setAddingAccount(true); setPatInput(""); setPatError(""); }, keepOpen: true });
  ...
  const settingsItems = useMemo(() => {
    const items: { id: string; action: () => void; keepOpen: boolean }[] = [
      ...
      { id: "settings-code-theme", action: () => switchMode("code-theme"), keepOpen: true },
      { id: "settings-model", action: () => switchMode("model"), keepOpen: true },
      { id: "settings-accounts", action: () => switchMode("accounts"), keepOpen: true },
    ];
    return items;
  }, [switchMode]);
  ...
  const filteredModels = useMemo(() => {
    ...
    return MODELS.filter(
      (m) => m.label.toLowerCase().includes(s) || m.desc.toLowerCase().includes(s) || m.id.toLowerCase().includes(s)
    );
  }, [mode, search, MODELS]);
  ...
  const allItems = mode === "commands" ? commandItems : mode === "search" ? searchItems : mode === "theme" ? themeItems : mode === "code-theme" ? codeThemeItems : mode === "accounts" ? accountItems : mode === "settings" ? settingsItems : mode === "model" ? modelItems : fileItems;
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent) => {
      ...
      if (e.key === "Backspace" && mode !== "commands" && !search) {
        ...
        switchMode(mode === "model" || mode === "code-theme" ? "settings" : "commands");
        return;
      }
                ...
                <div ref={listRef} className="overflow-y-auto max-h-[400px]">
                  {mode === "commands" ? (
                    <>
                          ...
                          {suggestions.map((item) => {
                            ...
                            return (
                              <CommandItemButton
                                ...
                                onClick={() => item.keepOpen ? item.action() : runCommand(item.action)}
                              >
                                {item.image ? (
                                  <img src={item.image} alt="" className="size-4 rounded-full shrink-0" />
                                ) : (
                                ...
                                <span className="text-[11px] text-muted-foreground/40 hidden sm:block">{item.description}</span>
                                {item.shortcut && (
                      ...
                      {hasQuery && repoNavItems.length > 0 && (
                      ...
                      {/* Recently viewed */}
                      {dedupedRecentViews.length > 0 && (
                        <CommandGroup title="Recently viewed">
                          {dedupedRecentViews.map((item) => {
                            const idx = getNextIndex();
                            return (
                              <CommandItemButton
                                key={item.url}
                      ...
                      {/* No results */}
                      {hasQuery && primaryTools.length === 0 && dedupedRecentViews.length === 0 && repoNavItems.length === 0 && (
                    ...
                    <>
                      {(() => {
                        const darkThemes = filteredCodeThemes.filter((t) => t.mode === "dark");
                        const lightThemes = filteredCodeThemes.filter((t) => t.mode === "light");
                        return (
                          <>
                            {darkThemes.length > 0 && (
                              <CommandGroup title="Dark">
                                {darkThemes.map((theme) => {
                                  const idx = getNextIndex();
                                  const isActive = codeThemeDark === theme.id;
                                  return (
                                    <CommandItemButton
                                      key={theme.id}
                                      index={idx}
                                      selected={selectedIndex === idx}
                                      onClick={() => setCodeThemeDark(theme.id)}
                                    >
                                      <span className="flex items-center gap-1 shrink-0">
                                        <span
                                          className="w-3 h-3 rounded-full border border-border/40"
                            ...
                            {lightThemes.length > 0 && (
                              <CommandGroup title="Light">
                                {lightThemes.map((theme) => {
                                  const idx = getNextIndex();
                                  const isActive = codeThemeLight === theme.id;
                                  return (
                                    <CommandItemButton
                                      key={theme.id}
                                      index={idx}
                                      selected={selectedIndex === idx}
                                      onClick={() => setCodeThemeLight(theme.id)}
                                    >
                                      <span className="flex items-center gap-1 shrink-0">
                                        <span
                                          className="w-3 h-3 rounded-full border border-border/40"
                      ...
                      {hasQuery && filteredCodeThemes.length === 0 && (
                        <div className="py-8 text-center text-sm text-muted-foreground/70">
                          No code themes match &quot;{search}&quot;
                        </div>
                      )}
                    </>
                  ) : mode === "accounts" ? (
                      ...
                      {accountsLoading && !accountsData ? (
                        ...
                        <>
                              ...
                              {(() => {
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <span className="text-[13px] text-foreground flex-1">Configuration</span>
                                    <span className="text-[11px] text-muted-foreground/40 hidden sm:block">Theme, model & more</span>
                              ...
                              {(() => {
                                ...
                                return (
                                  <CommandItemButton
                                    index={idx}
                                    selected={selectedIndex === idx}
                                    onClick={() => runCommand(() => window.open(`https://github.com/${activeLogin}`, "_blank"))}
                                  >
                                    <ExternalLink className="size-3.5 text-muted-foreground/50 shrink-0" />
                                    <span className="text-[13px] text-foreground">Go to GitHub Profile</span>
                                    <span className="text-[11px] text-muted-foreground/40 hidden sm:block">{activeLogin}</span>
                                  </CommandItemButton>
                                );
                              })()}
                              {(() => {
                                ...
                                return (
                                  <CommandItemButton
                                    index={idx}
                                    selected={selectedIndex === idx}
                                    onClick={() => { setAddingAccount(true); setPatInput(""); setPatError(""); }}
                                  >
                                    <Key className="size-3.5 text-muted-foreground/50 shrink-0" />
                                    <span className="text-[13px] text-foreground">Set PAT</span>
                                    <span className="text-[11px] text-muted-foreground/40 hidden sm:block">Rate limit fallback</span>
                                  </CommandItemButton>
                                );
                              })()}
                            </CommandGroup>
                          ...
                          <CommandGroup title="Switch Account">
                            ...
                            {(() => {
                              const matchesFilter = !search.trim() || accountsData.oauthLogin.toLowerCase().includes(search.toLowerCase());
                              ...
                              return (
                                ...
                                >
                                  {accountsData.oauthAvatar ? (
                                    <img src={accountsData.oauthAvatar} alt="" className="size-5 rounded-full shrink-0" />
                                  ) : (
                            ...
                            {accountsData.accounts.map((acc) => {
                              const matchesFilter = !search.trim() || acc.login.toLowerCase().includes(search.toLowerCase()) || acc.label.toLowerCase().includes(search.toLowerCase());
                              if (!matchesFilter) return null;
                              const idx = getNextIndex();
                              return (
                                <div key={acc.id} className="group/account relative">
                                  <CommandItemButton
                                    index={idx}
                                    selected={selectedIndex === idx}
                                    onClick={() => handleSwitchAccount(acc.id)}
                                  >
                                    <img src={acc.avatarUrl} alt="" className="size-5 rounded-full shrink-0" />
                                    <span className="text-[13px] text-foreground flex-1">
                                      {acc.login}
                                      {acc.label !== acc.login && (
                                        <span className="text-muted-foreground/40 ml-1.5 text-[11px]">{acc.label}</span>
                                      )}
                                    </span>
                                    <span className="text-[10px] font-mono text-muted-foreground/40 bg-muted/50 dark:bg-white/[0.04] px-1.5 py-0.5 rounded-sm">
                                      PAT
                              ...
                              <div className="px-4 py-2.5">
                                <div className="flex items-center gap-2">
                                  <input
                                    ...
                                    onKeyDown={(e) => {
                                      if (e.key === "Enter") { e.preventDefault(); e.stopPropagation(); handleAddAccount(); }
                                      if (e.key === "Escape") { e.stopPropagation(); setAddingAccount(false); }
                                    }}
                                  ...
                                  >
                                    {patSubmitting ? <Loader2 className="size-3 animate-spin" /> : "Add"}
                                  </button>
                                  <button
                                ...
                                {patError && (
                                  <p className="mt-1.5 text-[10px] font-mono text-destructive">{patError}</p>
                                )}
                                <p className="mt-1.5 text-[10px] font-mono text-muted-foreground/40">
                                  Paste a GitHub PAT to add another account.
                                </p>
                              </div>
                            ) : (
                              (() => {
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <span className="text-[13px] text-foreground">Add Account</span>
                                    <span className="text-[11px] text-muted-foreground/40 hidden sm:block">via Personal Access Token</span>
                                  </CommandItemButton>
                                );
                              })()
                            )}
                    ...
                    <>
                      <CommandGroup title="Configuration">
                        {(() => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <span className="text-[13px] text-foreground flex-1">Theme</span>
                              <span className="text-[11px] text-muted-foreground/40 hidden sm:block">
                                {colorThemes.find((t) => t.id === colorTheme)?.name ?? colorTheme}
                              </span>
                              <ChevronRight className="size-3 text-muted-foreground/30 shrink-0" />
                            </CommandItemButton>
                          );
                        ...
                        {(() => {
                          const idx = getNextIndex();
                          const activeDarkTheme = BUILT_IN_THEMES.find((t) => t.id === codeThemeDark);
                          return (
                            <CommandItemButton
                              key="settings-code-theme"
                              index={idx}
                              selected={selectedIndex === idx}
                              onClick={() => switchMode("code-theme")}
                            >
                              <Code className="size-3.5 text-muted-foreground/50 shrink-0" />
                              <span className="text-[13px] text-foreground flex-1">Code Theme</span>
                              <span className="text-[11px] text-muted-foreground/40 hidden sm:block">
                                {activeDarkTheme?.name ?? codeThemeDark}
                              </span>
                              <ChevronRight className="size-3 text-muted-foreground/30 shrink-0" />
                            </CommandItemButton>
                          );
                        ...
                        {(() => {
                          const idx = getNextIndex();
                          const currentModel = settingsData?.ghostModel ?? "";
                          const modelLabel = MODELS.find((m) => m.id === currentModel)?.label ?? currentModel.split("/").pop() ?? "—";
                          return (
                            <CommandItemButton
                              key="settings-model"
                              index={idx}
                            ...
                            >
                              ...
                              <span className="text-[13px] text-foreground flex-1">Ghost Model</span>
                              <span className="text-[11px] text-muted-foreground/40 hidden sm:block">
                                {settingsLoading ? "…" : modelLabel}
                              </span>
                              <ChevronRight className="size-3 text-muted-foreground/30 shrink-0" />
                            </CommandItemButton>
                          );
                        ...
                        {(() => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <span className="text-[13px] text-foreground flex-1">Account Settings</span>
                              <span className="text-[11px] text-muted-foreground/40 hidden sm:block">Profile, accounts & sign out</span>
                              <ChevronRight className="size-3 text-muted-foreground/30 shrink-0" />
                            </CommandItemButton>
                          );
                        })()}
                    ...
                    <>
                      {fileTreeLoading ? (
                        <div className="flex items-center justify-center py-8 gap-2">
                          <Loader2 className="size-4 text-muted-foreground/40 animate-spin" />
                          <span className="text-xs text-muted-foreground/40">Loading file tree...</span>
                        </div>
                      ) : filteredFiles.length > 0 ? (
                        <CommandGroup title={hasQuery ? "Files" : "Repository Files"}>
                    ...
                    <>
                      <CommandGroup title="Ghost Model">
                        {filteredModels.map((model) => {
                          const idx = getNextIndex();
                          const isActive = settingsData?.ghostModel === model.id;
                          return (
                            <CommandItemButton
                              key={model.id}
                              index={idx}
                              selected={selectedIndex === idx}
                              onClick={() => updateSetting({ ghostModel: model.id })}
                            >
                              <Bot className="size-3.5 text-muted-foreground/50 shrink-0" />
                              <span className="text-[13px] text-foreground flex-1">{model.label}</span>
                              <span className="text-[11px] text-muted-foreground/40 hidden sm:block">{model.desc}</span>
                              {isActive && (
                                <Check className="size-3.5 text-success shrink-0" />
                              )}
                            </CommandItemButton>
                      ...
                      {hasQuery && filteredModels.length === 0 && (
                        <div className="py-8 text-center text-sm text-muted-foreground/70">
